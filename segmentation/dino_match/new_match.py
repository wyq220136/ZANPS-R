...
try:
    from dino_match.render_score import compute_multiview_render_score
except Exception:
...
def _visible_cad_indices_from_gt_mask(obj_dir, frame_id, cad_meta, min_visible_pixels=30):
    ...
    if not os.path.isdir(gt_mask_dir):
        return list(range(len(cad_meta))), []
    visible = []
    invisible_ids = []
    for i, m in enumerate(cad_meta):
        ...
        found = False
        pix = 0
        cand_names = [f"mask_{pid}", f"{pid}", f"{pid:04d}"]
        for cn in cand_names:
            ...
            if found:
                break
        if (not found) or pix < int(min_visible_pixels):
...
def find_object_mask_path(obj_dir, frame_id):
    ...
    if not os.path.isdir(mask_dir):
        return None
    for ext in (".png", ".jpg", ".jpeg"):
        p = os.path.join(mask_dir, f"{frame_id}{ext}")
        if os.path.exists(p):
            return p
    return None
...
def build_cad_templates_with_aux(obj_dir, semantic_matcher, appearance_matcher):
    ...
    gt_mask_root = os.path.join(obj_dir, "gt_mask")
    if not os.path.isdir(models_root):
        raise FileNotFoundError(f"models folder not found: {models_root}")
    if not os.path.isdir(gt_mask_root):
        raise FileNotFoundError(f"gt_mask folder not found: {gt_mask_root}")

    frame_ids = sorted(
        [d for d in os.listdir(gt_mask_root) if os.path.isdir(os.path.join(gt_mask_root, d))],
        key=natural_sort_key,
    )
    if len(frame_ids) == 0:
        raise RuntimeError(f"No frame folders in {gt_mask_root}")
    ...
    for view_id, (model_root_for_view, model_entries) in view_to_models.items():
        ref_frame_id = select_ref_for_view(view_id if view_id >= 0 else 0)
        ref_rgb_path = find_rgb_path(obj_dir, ref_frame_id)
        ref_mask_dir = os.path.join(gt_mask_root, ref_frame_id)
        if ref_rgb_path is None or (not os.path.isdir(ref_mask_dir)):
        ...
        mask_files = load_mask_files(ref_mask_dir)
        if len(mask_files) == 0:
        ...
        for mf in mask_files:
            mp = os.path.join(ref_mask_dir, mf)
...
def run_matching_for_object(
    ...
    try:
        templates_by_view = build_cad_templates_with_aux(
            obj_dir,
            semantic_matcher,
            appearance_matcher,
        )
    ...
    for frame_id in frame_ids:
        ...
        render_sem_matrix = np.zeros_like(base_matrix, dtype=np.float32)
        render_appe_matrix = np.zeros_like(base_matrix, dtype=np.float32)
        pose_cache = {}
        # Stage-1 (SAM6D-like): only score and rank all CAD<->mask pairs.
        # One-to-one assignment is deferred to point-cloud rerank stage.
        local_to_global = {int(i_local): int(i_global) for i_local, i_global in enumerate(candidate_idx.tolist())}

        # Keep SAM6D candidate ranking pure. Render scoring is applied later
        # inside adaptive_weight after point-cloud alignment provides a pose.
                ...
                frame_res.append(
                    {
                        ...
                        "render_semantic_score": float(render_sem_matrix[int(i_local), int(c_idx)]),
                        "render_appearance_score": float(render_appe_matrix[int(i_local), int(c_idx)]),
                        ...
                        "score": float(score),
                        "best_render_view_idx": int(pose_cache[(int(i_local), int(c_idx))]["view_index"]) if (int(i_local), int(c_idx)) in pose_cache else -1,
                        "coarse_pose": matrix_to_nested_list(pose_cache[(int(i_local), int(c_idx))]["pose"]) if (int(i_local), int(c_idx)) in pose_cache else None,
                        "refined_pose": None,
                    }
                )
