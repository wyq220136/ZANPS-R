...
def build_parser():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Use obj/gt_mask frame masks directly as candidate masks for CAD-mask matching, skipping SAM.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Top-K masks per CAD from SAM6D ranking used in point-cloud one-to-one matching stage.",
    )
    ...
    parser.add_argument("--skip-adaptive-weight", action="store_true", help="Skip point-cloud adaptive rerank stage")
...
def main():
    ...
    if args.use_gt_mask_for_match:
        print("[STAGE-1/GT-MASK] using gt_mask as direct_match candidate masks")
