...
METHOD = "instantmesh_tsdf_dmesh"
BASE_METHOD = "instantmesh"
TSDF_METHOD = "instantmesh_tsdf"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        "Run InstantMesh + TSDF fusion + legacy dmesh-branch DLMesh refinement using shared caches.",
...
# Usage:
#   python reconstruction/run/recon_instantmesh_tsdf_dmesh.py --data-root dataset_train --split val --work-root reconstruction_runs --objects bottle_3517 --gpus 0 --num-workers 1
#   python reconstruction/run/recon_instantmesh_tsdf_dmesh.py --data-root /data/dataset_train --split val --work-root /shared/recon_runs --object-source all --gpus 0,1 --num-workers 2 --mode multi_image --coord-dir /shared/recon_coord/instantmesh_tsdf_dmesh --reset-coord
#
# Pipeline:
#   1. Reads/builds <work-root>/instantmesh.
#   2. Writes/reuses <work-root>/instantmesh_tsdf.
#   3. Runs remesh + PyTorch3D pose optimization + DLMesh refinement from <work-root>/instantmesh_tsdf into <work-root>/instantmesh_tsdf_dmesh.
#
# Key parameters:
#   --work-root: shared cache root for all stages.
#   --build-base-if-missing: run InstantMesh first if its shared cache is absent before TSDF.
