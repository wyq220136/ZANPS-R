...
class PoseRefinePredictor:
  def __init__(self,):
    ...
    train_cfg_path = os.path.join(code_dir, 'configs', 'train_ddp.yaml')
    if os.path.isfile(train_cfg_path):
      train_cfg = OmegaConf.load(train_cfg_path)
      if 'vm_weight' in train_cfg:
        self.cfg['vm_weight'] = train_cfg['vm_weight']
        logging.info(f"Loaded vm_weight={self.cfg['vm_weight']} from {train_cfg_path}")
    if 'vm_weight' not in self.cfg:
      self.cfg['vm_weight'] = 0.75
    self.cfg['vm_weight'] = float(np.clip(float(self.cfg['vm_weight']), 0.0, 1.0))
  ...
  def predict(self, rgb, depth, K, ob_in_cams, xyz_map, normal_map=None, get_vis=False, mesh=None, mesh_tensors=None, glctx=None, mesh_diameter=None, iteration=5):
    ...
    for _ in range(iteration):
      ...
      for b in range(0, pose_data.rgbAs.shape[0], bs):
        ...
        if prev_validity_mask is not None:
          vm = prev_validity_mask[b:b+bs]
          if vm.shape[-2:] != A.shape[-2:]:
            vm = F.interpolate(vm, size=A.shape[-2:], mode='bilinear', align_corners=False)
          vm = vm.clamp(0.0, 1.0)
          # Focus updates on high-confidence rendered regions while keeping global context.
          vm_weight = float(np.clip(float(self.cfg.get('vm_weight', 0.75)), 0.0, 1.0))
          A = A * ((1.0 - vm_weight) + vm_weight * vm)
