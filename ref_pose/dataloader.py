...
class QueryLoader:
  def __init__(self,files_dir, downscale=1, shorter_side=None, zfar=np.inf):
    self.video_dir = files_dir
    self.downscale = downscale
    self.zfar = zfar
    self.color_files = sorted(glob.glob(f"{self.video_dir}/rgb/*.png"))
    self.K = np.loadtxt(f'{files_dir}/cam_K.txt').reshape(3,3)
    self.id_strs = []
...
class SingleLoader:
  def __init__(self, rgb_path, mesh_dir, mask_dir, K, depth_path, zfar=np.inf, use_nvdiffrast=True, refiner_mode="validity_mask"):
    self.use_nvdiffrast = use_nvdiffrast
    self.refiner_mode = refiner_mode
    self.scorer = ScorePredictor() if self.use_nvdiffrast else None
    self.refiner = self._make_refiner() if self.use_nvdiffrast else None
    self.glctx = None
    if self.use_nvdiffrast:
      try:
        ...
        logging.warning(f"Failed to initialize NvDiff context, fallback to no NvDiff: {e}")
        self.use_nvdiffrast = False
    self.zfar = zfar
    self.reinit(rgb_path, mesh_dir, mask_dir, K, depth_path)

  def _make_refiner(self):
    if self.refiner_mode == "original":
      from ref_pose.ablation.predict_pose_refine_original import OriginalPoseRefinePredictor
      return OriginalPoseRefinePredictor()
    if self.refiner_mode == "validity_mask":
      from learning.training.predict_pose_refine import PoseRefinePredictor
      return PoseRefinePredictor()
    raise ValueError(f"unknown refiner_mode: {self.refiner_mode}")
    
 
  def get_mask(self, i):
    mask = cv2.imread(self.masks[i],-1)
    if len(mask.shape)==3:
      for c in range(3):
        if mask[...,c].sum()>0:
          mask = mask[...,c]
  ...
  def get_mesh(self, i):
    parts = self.models[i]
    mesh = trimesh.load(parts, force="mesh")
    return mesh
  
    
  def get_rgb(self):
    color = imageio.imread(self.rgb_path)[...,:3]
    color = cv2.resize(color, (self.W,self.H), interpolation=cv2.INTER_NEAREST)
    return color, self.base_name
    
    
  def reinit(self, rgb_path, mesh_dir, mask_dir, K, depth_path):
    self.base_name = os.path.basename(rgb_path)
    self.models = []
    self.masks = []
    self.part_ids = []
    self.init_poses = []
    ...
    for name in sorted(os.listdir(mesh_dir), key=natural_sort_key):
      model_dir = os.path.join(mesh_dir, name)
      ...
      mask_path = os.path.join(mask_dir, f"mask_{part_id}.png")
      if not os.path.exists(mask_path):
        print(f"Warning: mask not found for part {part_id}: {mask_path}")
        continue

      raw_pose_path = os.path.join(model_dir, "raw_pose.txt")
      init_pose = np.eye(4, dtype=np.float32)
      if os.path.exists(raw_pose_path):
        try:
          raw = np.loadtxt(raw_pose_path, dtype=np.float32)
          if raw.shape == (16,):
            raw = raw.reshape(4, 4)
          if raw.shape == (4, 4):
            init_pose = raw.astype(np.float32)
        except Exception as e:
  ...
  def get_depth(self):
    depth = cv2.imread(self.depth_path,-1)/1e3
    depth = cv2.resize(depth, (self.W,self.H), interpolation=cv2.INTER_NEAREST)
    depth[(depth<0.1) | (depth>=self.zfar)] = 0
    return depth

  def get_part_id(self, i):
    return self.part_ids[i]


  def get_init_pose(self, i):
...
class BopBaseReader:
  ...
  def get_mask(self, i_frame:int, ob_id:int, type='mask_visib'):
    ...
    if self.scene_gt is not None:
      ...
      # print(f'[get_mask] i_frame={i_frame}, ob_id={ob_id}, mask_path={mask_path}, exists={os.path.exists(mask_path)}')
      raise RuntimeError
...
class LinemodOcclusionReader(BopBaseReader):
  ...
  def get_gt_mesh_file(self, ob_id):
    mesh_dir = f'{BOP_DIR}/{self.dataset_name}/models/ob_{ob_id:06d}.ply'
    return mesh_dir
